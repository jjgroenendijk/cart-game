...
5. Shell-lint tools: install `shellcheck` and `shfmt` from the distro repo
...
### PR-body footer on web-session creation

The `attribution` block above is honored for commits (no `Claude-Session:`
trailer appears), but a Claude Code on the web session can still append a
`Generated by Claude Code` footer with the session link to a PR body when the
PR is opened through the GitHub MCP `create_pull_request` path -> that create
path does not apply `attribution.pr`/`sessionUrl`. The footer is added
server-side after the create call, so passing a clean body does not prevent it.

Workaround: after opening a PR, edit the body once via `update_pull_request`
(or on GitHub) to remove the footer. The append only happens on create, so an
update passes the body through verbatim and the footer does not return. Opening
a PR here always includes this strip-after-create step.
