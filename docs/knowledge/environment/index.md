# Environment

- [cascade](/environment/cascade.md) — Environment update order and biome bias
  cascade
- [clouds](/environment/clouds.md) — Procedural cloud system
- [day-cycle](/environment/day-cycle.md) — Pure day/night model and the shared
  singleton state read by sky, lights, weather, and post-grade
- [dressing](/environment/dressing.md) — Procedural prop placement, flora
  registry, and chunk streaming
- [dynamic-sky](/environment/dynamic-sky.md) — Day/night cycle with sun, moon,
  stars, and fog
- [flora-archetypes](/environment/flora-archetypes.md) — Flora kind definitions
  and rendering
- [prop-factory](/environment/prop-factory.md) — Prop mesh generation and caching
- [prop-sampler](/environment/prop-sampler.md) — Prop placement sampling strategies
- [sun-disc](/environment/sun-disc.md) — Sun disc rendering and atmospheric
  scattering
- [water](/environment/water.md) — Depth-aware cel-shaded water plane with shore
  foam and sun glint
- [weather](/environment/weather.md) — GPU particle weather system with seeded
  deterministic director
- [wildlife](/environment/wildlife.md) — Wildlife spawning and behavior
