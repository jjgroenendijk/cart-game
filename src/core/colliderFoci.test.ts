...
// (view) and .group.position (rivals). Cast through unknown so the test needs
...
describe("fillKartFoci (202 collider foci pool)", () => {
  it("writes the human view then rivals in order", () => {
    const out: Pt[] = [];
  ...
  it("reuses pooled slots and truncates when the kart count shrinks", () => {
    const out: Pt[] = [];
    fillKartFoci(out, view(1, 1, 1), [rival(2, 2, 2), rival(3, 3, 3)]);
  ...
  it("returns a single-element pool with just the human when rivals empty", () => {
    const out: Pt[] = [];
    fillKartFoci(out, view(7, 7, 7), []);
