...
describe("selectWeatherPreset", () => {
  ...
  it("leafFall is selectable when weighted (sole weight -> always leafFall)", () => {
    for (const seed of [0, 1, 42, 99, 200]) {
      expect(selectWeatherPreset({ leafFall: 1 }, seed)).toBe("leafFall");
    }
  });

  it("appending leafFall does not perturb the default clear/rain/snow partition", () => {
    // Parity guard: leafFall carries no DEFAULT_WEATHER_WEIGHTS key, so the
    // cumulative walk over PRESET_ORDER stays bit-identical to pre-leafFall.
    const reached = new Set<WeatherPreset>();
    for (let seed = 0; seed <= 200; seed++) {
      reached.add(selectWeatherPreset(DEFAULT_WEATHER_WEIGHTS, seed));
    }
    expect(reached.size).toBe(3);
    expect(reached.has("leafFall")).toBe(false);
  });
});

describe("leafFall preset config", () => {
  it("has a WEATHER_PRESET_CONFIG entry", () => {
    expect(WEATHER_PRESET_CONFIG.leafFall).toBeDefined();
  });

  it("is soft (fuzzy blob + vertex sway) and drifts slowly with high lateral drift", () => {
    const cfg = WEATHER_PRESET_CONFIG.leafFall;
    expect(cfg.soft).toBe(true);
    expect(cfg.fall).toBeGreaterThan(-3); // slow, drifting fall (not driving rain)
    expect(cfg.fall).toBeLessThan(0); // still downward
    expect(cfg.drift).toBeGreaterThanOrEqual(3); // high lateral tumble
  });
});
